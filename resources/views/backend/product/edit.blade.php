    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('product.update', $product->id) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="inputPhoto" class="col-form-label">Foto Produk</label>
                    ...
                    <small class="form-text text-muted">
                        ...
                        - Ukuran maksimal per foto: 2MB<br>
                        ...
                        <div style="margin-top:15px;">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                                @php
                                    ...
                                    if (!is_array($photos)) {
                                        $photos = [$product->photo]; // fallback untuk data lama
                                    }
                                @endphp
                                @foreach ($photos as $index => $photo)
                                    <div style="position: relative; display: inline-block;">
                                        <img src="{{ $photo }}"
                                            style="width: 120px; height: 120px; object-fit: cover; border: 2px solid #ddd; border-radius: 8px; margin: 2px;">
                                        <div
                                            style="text-align: center; font-size: 11px; font-weight: bold; margin-top: 3px; color: #555;">
                                            Foto {{ $index + 1 }}
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <div id="imagePreview" style="margin-top:15px; display: flex; flex-wrap: wrap; gap: 10px;">
                        <!-- Preview foto baru akan muncul di sini -->
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        ...
                        <option value="inactive" {{ $product->status == 'inactive' ? 'selected' : '' }}>Tidak Aktif
                        </option>
                    </select>
                    @error('status')
                        <span class="text-danger">{{ $message }}</span>
    ...
    <script>
        ...
        function previewMultipleImages(input) {
            ...
            if (input.files && input.files.length > 0) {
                ...
                totalInfo.innerHTML =
                    `<strong>Total: ${input.files.length} foto baru dipilih (akan mengganti semua foto lama)</strong>`;
                totalInfo.style.width = '100%';
                totalInfo.style.textAlign = 'center';
                totalInfo.style.marginTop = '10px';
                totalInfo.style.padding = '10px';
                totalInfo.style.backgroundColor = '#fff3cd';
                totalInfo.style.border = '1px solid #ffc107';
                totalInfo.style.borderRadius = '4px';
                totalInfo.style.color = '#856404';
